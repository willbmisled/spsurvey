spsurvey finite_design vignette by Thomas Kincaid
========================================================
##Document source: [Finite_Design.pdf](http://cran.r-project.org/web/packages/spsurvey/vignettes/Finite_Design.pdf)

NOTE:  either a shapefile or an sp object can be used for the sampling frame input.  However, it must be a point coverage, not a polygon.
...
```{r preliminaries, include=TRUE, echo=TRUE, cache=TRUE}
  library(spsurvey)  #load package
  setwd("./vignettes")
  att <- read.dbf("NE_lakes") # Read the attribute table from the shapefile
  head(att) # Display the initial six lines in the attribute data frame
  ...
  #Plot lakes in the southern New England region
    plot(att$xcoord, att$ycoord, xlab="x-coordinate", ylab="y-coordinate", pch=20,cex=0.1, col="red")
```
...
```{r equalSites, include=TRUE, echo=TRUE, cache=TRUE}
  # Call the set.seed function so that the survey designs can be replicate
    set.seed(4447864)
  # Create the design list
    Equaldsgn <- list(None=list(panel=c(PanelOne=300), seltype="Equal")) #PanelOne=300 is the sample size
  # Select the sample
    Equalsites <- grts(design=Equaldsgn, #design file specified above
                   DesignID="EQUAL",     #prefix for siteID
                   type.frame="finite",  #type of sample frame=finite
                   src.frame="shapefile",#source of the sample frame-in this case a shapefile
                   in.shape="NE_lakes",  #input shapefile
                   att.frame=att,        #data frame attributes;see above;this is the shapefile attributes
                   shapefile=FALSE)      #whether to output the results as a shapefile
  # Print the initial six lines of the survey design
    head(Equalsites@data)
  # Print the survey design summary
    dsgnsum(Equalsites)  #this can take a while so wait
```
